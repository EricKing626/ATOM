...
class ModelRunner:
    ...
    def __init__(self, rank: int, config: Config):
        ...
        self._cg_probe = os.environ.get("ATOM_CG_PROBE", "") == "1"
    ...
    def _estimate_cudagraph_overhead(self):
        ...
        # V4-Pro TP8 (CG-PROBE): a single q-bucket graph set across all bs adds
        ...
        if hasattr(self, "drafter") and getattr(
            ...
            sum_q = sum(buckets)
            # Estimate the graph pool from the graph SHAPES (TP/DP-invariant; do
            # NOT scale by the warmup activation, which differs ~2x TP vs DP for
            # the SAME graphs -> 70GB vs 150GB, negative KV under DP). Each
            # q-bucket's pool ~ its largest captured graph (max_bs * q); sum over
            # buckets -> ~per_q * Σq. per_q calibrated to a DSV4 8-bucket ~65GB.
            hidden = int(self.config.hf_config.hidden_size)
            max_bs = int(self.config.max_num_seqs)
            per_q = 1.4 * (1 << 30) * (max_bs / 512.0) * (hidden / 7168.0)
            overhead = 1.3 * per_q * sum_q
            logger.info(
                "DSpark cudagraph mem estimate: buckets=%s n=%d Σq=%d "
                "max_bs=%d hidden=%d -> overhead=%.2fGB",
                buckets, n_buckets, sum_q, max_bs, hidden,
                overhead / (1 << 30),
            )
        return int(overhead)
    ...
    def _dspark_sync_graph_shape_dp(self, batch: ScheduledBatch) -> None:
        ...
        if self.config.parallel_config.data_parallel_size <= 1:
            return
        # Only DSpark's DYNAMIC verify length makes (bs, q) diverge across ranks.
        # Plain MTP has a fixed q (= mtp_k+1) already uniform on every rank, so
        # the sync is unnecessary there — skip it to avoid an extra per-step
        # all_reduce that would slow MTP decode.
        if not (
            hasattr(self, "drafter")
            and getattr(self.drafter, "dspark_confidence_schedule", False)
        ):
        ...
        if not forward_mode.use_cudagraph:
            ...
            with record_function(label):
                ...
                if self._piecewise_cg_active():
                    ...
                    # sized to the REAL ragged token count Σ (= total_tokens_num_
                    # decode) instead of the rectangular C = bs*q_eff -> MoE/linear
                    # shrink with Σ (DSpark dynamic). Attention is eager on the
                    # flat [0:Σ] tokens; the [Σ:pad] tail is masked (batch_id -1).
                    #
                    # DP is EXCLUDED: per-rank Σ diverges -> different buckets ->
                    # the fixed-size MoE all_gather deadlocks/accumulates. DP uses
                    # the rectangular bucket (and should run FULL for decode).
                    _one_d = (
                    ...
                    if _one_d and not _is_dummy:
                        # TP-only (guarded above): all ranks share the same batch
                        # -> same Σ -> num_tokens_pad naturally identical, so NO
                        # cross-rank sync is needed. (A per-step gloo all_reduce
                        # here deadlocked the async GPU pipeline -> removed.)
                        ...
                        if _i < len(_bk):
                            ...
                            _captured = False
                        # Pad tail to a LEGAL vocab id / position (the builder
                        # fills to graph_cap >= num_tokens_pad, so this is a no-op
                        # safety net; matches the working baseline).
                    ...
                    if self.use_aux_hidden_state_outputs:
                        ...
                        hidden_states = model_output
                        self._aux_hidden_states = None
                    self._collect_dspark_aux(num_tokens_pad)
                    # Slice off the pad tail before sampling. The forward ran on
                    # the padded [0:num_tokens_pad] window (so the captured piece
                    # shapes match), but rows [real:num_tokens_pad] are pad garbage
                    # and must not reach the sampler.
                    #
                    # non-spec: the real length is num_tokens = scheduled_bs *
                    # max_q_len (max_q_len == 1), identical to the length the
                    # non-PIECEWISE path uses at forward_vars["outputs"][:
                    # num_tokens]. Previously this sliced to num_tokens_pad, so
                    # pad rows leaked into sampled_token_ids (len = pad, e.g. 256)
                    # -> became prev_token_ids -> next step's prepare_input_ids
                    # assigned it into a deferred-sized slot (e.g. 253) -> crash.
                    #
                    # spec (DSpark): keep the padded slice UNCHANGED — the ragged
                    # postprocess/draft indices reference the flat [0:Σ] layout
                    # with Σ <= num_tokens_pad and re-gather to bs via
                    # next_token_locs, so the tail rows are masked downstream.
    ...
    def capture_cudagraph(self):
        ...
        # PIECEWISE 1D-ragged: pieces are per-token. We do NOT expand q here (that
        # would capture ~len(graph_bs)*full_q num_tokens buckets -> pool OOM).
        # Instead the natural {graph_bs * q_eff} products already form a coarse
        # ~geometric num_tokens grid; run_model replays at round_up_1d(Σ) over
        # whatever buckets we capture. Set ATOM_PIECEWISE_FINE_TOKENS=1 to also
        # capture q=1..full_q for a tighter (but far heavier) grid.
        if (
            _piecewise
            and os.environ.get("ATOM_PIECEWISE_FINE_TOKENS", "0") == "1"
            and full_q_len > 1
        ):
            q_buckets = sorted(set(q_buckets) | set(range(1, full_q_len + 1)))
            logger.info("PIECEWISE FINE piece num_tokens grid: q=%s", q_buckets)
        ...
        with pause_gc(), graph_capture() as capture_ctx:
          for max_q_len in q_buckets:
            ...
            # Capture LARGEST bs first (graph_bs is sorted descending) — same order
            # as the known-good build and vLLM (large shapes first). The memory
            # guard below uses a conservative per-token fallback so it can skip
            # oversized buckets on the very first (largest) visit without needing a
            # prior measurement; measurements only tighten logging.
            capture_range = (
        ...
        if _piecewise and self.rank == 0:
            ...
            # (esp. for DP, where the est must grow). Σtok = sum of captured
            # num_tokens; per_token_actual = pool / Σtok.
